...
void DirectXBase::CreateDeviceResources()
{
	...
	if (!m_isSupportGS)
	{
		...
		// If the device does not support Geometry shader, fall back to the WARP device.
...
void DirectXBase::CheckSupportGS()
{
	UINT creationFlags = D3D11_CREATE_DEVICE_BGRA_SUPPORT;
	D3D_FEATURE_LEVEL featureLevels[] =
	{
	...
	HRESULT hr = D3D11CreateDevice(
		...
		ARRAYSIZE(featureLevels),
		D3D11_SDK_VERSION, // Always set this to D3D11_SDK_VERSION for Windows Store apps.
		nullptr, // Set null to the Direct3D device.
		&m_d3dFeatureLevel, // Returns feature level of device supported.
		nullptr // Set null to device immediate context.
		);

	if (0 != m_d3dFeatureLevel && m_d3dFeatureLevel < D3D_FEATURE_LEVEL_10_0)
	{
		m_isSupportGS = false;
	}
}
...
void DirectXBase::UpdateForWindowSizeChange()
{
	...
	if (m_dpi != currentDisplayInformation->LogicalDpi)
	{
		return;
	}
